...
if (url.pathname === "/api/payments/freekassa/webhook") {

    /*
     * GET ничего не оплачивает.
     * Нужен только для проверки доступности URL.
     */
...
if (
    ...
    return new Response(
        ...
        <head>
            ...
            <title>Оплата успешна — RAUDA ILM</title>
        </head>
        <body style="
            margin:0;
            background:#111;
        ...
        ">
            <div>
                ...
                <p>Проверяем оплату и открываем доступ.</p>
                <a
                    href="/"
                    style="color:#f5c542"
                >
                    Вернуться в RAUDA ILM
                </a>
            </div>
        </body>
        </html>`,
...
if (
    ...
    return new Response(
        ...
        <head>
            ...
            <meta name="viewport"
                  content="width=device-width,initial-scale=1">
            <title>Оплата не завершена — RAUDA ILM</title>
        </head>
        <body style="
            margin:0;
            background:#111;
        ...
        ">
            <div>
                ...
                <p>Деньги не были подтверждены.</p>
                <a
                    href="/"
                    style="color:#f5c542"
                >
...
async function handleFreeKassaWebhook(
    request,
    env
) {
    try {
        ...
        if (
            ...
            !env.FREEKASSA_SECRET2
        ) {
            console.error(
                "FreeKassa secrets are not configured"
            );

            return new Response(
                "FREEKASSA_NOT_CONFIGURED",
                {
                    status: 503,
                    headers: {
                        "Content-Type":
        ...
        /*
         * Некоторые проверки URL могут
         * отправлять пустой запрос.
         *
         * Здесь НИКАКОЙ доступ не выдаётся.
         */
        if (
            !merchantId &&
            !amount &&
         ...
         * Подпись настоящая.
         *
         * Пока НЕ отвечаем YES на реальный
         * платёж, потому что ещё не сделали
         * безопасную привязку:
         *
         * заказ → пользователь → семестр.
         *
         * Так FreeKassa сможет повторить
         * уведомление после следующего этапа.
         */
        console.log(
            "FreeKassa payment verified",
            {
                orderId,
                operationId,
                amount
